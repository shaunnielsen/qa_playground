...
function getNumber(numberHandler) {
  ...
  //parameter we gibve the numberChecker
  funciton numberChecker(result: number) {
      ...
      describe("numberHandler", () => {
          ...
          it("looks good when checked by an anonymous callback", ( => {
              // the anonymous callback works exaclty the same way as the named callback.
              // the only real negative is the lack of reusability.
              getNumber((result) => {
                  expect(result >= 1).toBeTruthy();
                  expect(result <= 10).toBeTruthy();
                  expect(result.toFixed(0)).toBe(`${result}`);
